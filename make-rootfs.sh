...
function install_lab_certification_tools() {
    ...
    lab_commands=(
        met-sgs-rf-stop-services
        met-sgs-rf-run-tool
        met-sgs-rf-test
    )
    for command_name in "${lab_commands[@]}"; do
        lab_command="scripts/${command_name}"
        private_lab_command="${LAB_CERTIFICATION_TOOLS_SRC_DIR}/sgs/${command_name}"
        if [ ! -f "${private_lab_command}" ]; then
            echo "The private lab tools checkout is missing ${private_lab_command}."
            exit 1
        fi
        if ! cmp -s "${lab_command}" "${private_lab_command}"; then
            echo "${command_name} differs between meticulous-machine and lab-certification-tools."
            exit 1
        fi
    done
